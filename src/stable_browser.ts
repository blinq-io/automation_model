...
class StableBrowser {
  ...
  async closeUnexpectedPopups(info, _params) {
    if (
      this.configuration.popupHandlers &&
      this.configuration.popupHandlers.length > 0
    ) {
      if (!info) {
        info = {};
      }
      ...
      if (result.foundElements.length > 0) {
        ...
        let dialogCloseLocator = this._getLocator(
          this.configuration.popupHandlers[result.locatorIndex]
            .close_dialog_locator,
          scope,
          _params
        );
  ...
  async _locate(selectors, info, _params?: Params, timeout = 30000) {
    for (let i = 0; i < 3; i++) {
      let element = await this._locate_internal(
        selectors,
        info,
        _params,
        timeout
    ...
    switch (type) {
      case "users":
        ...
        const users = JSON.parse(
          fs.readFileSync(path.join(this.project_path, "users.json"), "utf8")
        );
  ...
  async waitForPageLoad(options = {}, world = null) {
    ...
    if (
      !(this.configuration && this.configuration.domcontentloaded === false)
    ) {
      promiseArray.push(
        createTimedPromise(
